...
namespace UnityEngine.InputLegacy.OldInputCompatibility
{
    ...
    {
        public static KeyCode NameToKey(string name)
        {
            if (s_NameToKey == null)
            {
                s_NameToKey = new Dictionary<string, KeyCode>();
                for (var i = 0; i < s_KeyToName.Length; i++)
                    s_NameToKey[s_KeyToName[i]] = (KeyCode) i;
            }

            return s_NameToKey.TryGetValue(name, out var keyCode) ? keyCode : KeyCode.None;
        }

        public static string KeyToName(KeyCode keyCode)
        {
            return s_KeyToName[(int)keyCode];
        }

        private static Dictionary<string, KeyCode> s_NameToKey;

        private static string[] s_KeyToName =
        {
            "", // SDLK_FIRST
            "",
            "",
